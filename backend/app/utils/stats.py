...
def compute_monthly_stats(db: Session, user_id: int, year: int, month: int) -> MonthlyStats:
    ...
    steps_habit = habit_crud.get_habit_by_key(db, user_id, STEPS_KEY)
    avg_steps = None
    steps_goal_days = 0
    if steps_habit:
    ...
    metrics = body_metric_crud.list_metrics(db, user_id, start, end)
    ...
    all_habits = habit_crud.list_habits(db, user_id)
    all_logs = habit_crud.list_all_logs_in_range(db, user_id, start, end)

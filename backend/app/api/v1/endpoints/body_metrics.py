...
def list_metrics(
    ...
    return crud.list_metrics(db, user.id, start, end)


@router.post("", response_model=BodyMetricRead, status_code=201)
def upsert_metric(data: BodyMetricUpsert, db: DbSession, user: CurrentUser):
    return crud.upsert_metric(db, user.id, data)
...
def get_weekly_average(
    ...
    metrics = crud.list_metrics(db, user.id, start, end)
    return weekly_averages(metrics)


@router.delete("/{metric_id}", status_code=204)
def delete_metric(metric_id: int, db: DbSession, user: CurrentUser):
    metric = crud.get_metric(db, user.id, metric_id)
    if not metric:
